...
def download(api_key, user, name, destination, data):
    try:
        ...
        if name == '*':
            ...
            for droplet in droplets:
                ...
                output = subprocess.run(f"scp -r {user}@{droplet}:{data} {destination}", text=True, check=True, shell=True, capture_output=True)
                print(output.stdout)
    ...
    # Errors
        ...
        print(f"{colors.red}ERROR IN: download: {e}{colors.reset}")
        sys.exit(1)
